...
- `POST /api/platform/events/join`：参加赛事活动；每位真实用户选 1 个已部署球球，2 位真实用户即可开赛并返回完整回放
...
- 竞赛场提供“参加赛事”入口。
- 赛事按真实邮箱用户计数，至少 2 位真实用户拥有已部署球球即可开赛。
- 系统会为每位用户选择 1 个已部署球球参赛，赛事结果写入完整对战记录并可直接进入回放。
- 旧的无邮箱种子用户和 `example.com` 测试用户会在平台状态归一化时被清理，相关球球和历史对局同步移除。
